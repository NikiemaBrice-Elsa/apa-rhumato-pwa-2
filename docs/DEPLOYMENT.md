...
**Mise à jour du 14/09/2026 (Sprint 24, Priorité 5 finitions techniques)** : la file d'attente (`offlineQueue.ts`) était déjà générique par entité dès le Sprint 12 (`entityType`, méthode HTTP, URL, corps) mais seule la séance (`SessionFlow.tsx`) s'en servait — les mesures de suivi (`SuiviFlow.tsx`, §35-§38) et le profil patient (`PatientProfileForm.tsx`, §13) échouaient simplement hors connexion (pour les mesures, sans même `try`/`catch` autour du `fetch`). Les deux écrans utilisent désormais la même file. Le type `QueuedOperation.method` a été étendu (`"PUT"` ajouté, pour `PUT /api/profile`).
...
Le Sprint 14 ajoute l'abonnement premium (§47, §48, page `/abonnement` + `/admin/abonnements`) : un patient demande un plan, transfère lui-même via Mobile Money/Orange Money/Moov Money selon les instructions affichées, déclare la référence de transaction reçue, puis un administrateur vérifie réellement ce transfert et confirme dans `/admin/abonnements` — ce qui active la souscription. **Aucune passerelle de paiement n'est intégrée** (voir `docs/DECISIONS.md`, §47 : « ne pas coder un système de paiement fictif ») : `SUPABASE_SERVICE_ROLE_KEY` (déjà requise depuis le Sprint 13) est également utilisée par le flux de confirmation de paiement. **Fait (corrigé le 13/09/2026, cette ligne était obsolète)** : le champ « Instructions de paiement » de chaque plan premium (`payment_instructions_fr`) est renseigné avec les numéros Mobile Money/Orange Money/Moov Money réels de Dr Nikiema depuis une migration exécutée début septembre 2026 — les patients voient déjà une instruction de paiement réelle à l'écran, aucune action supplémentaire n'est requise sur ce point avant l'ouverture de vraies inscriptions premium. La migration `0011_subscriptions_and_payments.sql` doit être appliquée (tables `subscription_plans`, `subscriptions`, `payments`, RLS sans policy `UPDATE` pour les patients — voir `docs/DECISIONS.md`).

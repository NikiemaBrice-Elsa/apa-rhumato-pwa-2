...
-- ===== MIGRATION : 0019_exercise_audio_two_clips.sql (ajoutée le 2026-09-10) =====
...
-- docs/DECISIONS.md, FEUILLE_DE_ROUTE_20260821.md section 4bis). Sa réponse
-- du 10/09/2026 choisit deux clips audio par exercice plutôt qu'un seul
-- (audio de préparation, puis audio pendant l'exercice, conformément à la
-- maquette initiale du document "contenu audio APAS en rhumato.docx").
--
-- Vérifié avant exécution : `audio_url` (Sprint 5) est vide pour les 8
-- exercices déjà validés (count=0/8), le renommage ne perd donc aucune
-- donnée. Exécuté et vérifié en production le 10/09/2026 : les colonnes
-- `audio_exercise_url` et `audio_preparation_url` existent bien, toutes
-- deux de type `text`.
--
-- Périmètre V1 : uniquement la partie principale — aucun contenu
-- d'échauffement/retour au calme n'existe (Dr Nikiema a explicitement
-- choisi de ne pas le fournir pour l'instant, Question 1 du 09/09/2026)
-- donc aucun audio de ce type ne sera construit tant qu'il ne sera pas
-- soumis séparément et validé. Aucun contenu audio n'est inséré par cette
-- migration : Dr Nikiema enregistrera lui-même (Question 5 du 09/09/2026)
-- et déposera les liens via l'écran d'administration des exercices.
...
comment on column public.exercise_library.audio_preparation_url is 'Coach vocal (Sprint 20) : audio joué avant l''exercice.';
